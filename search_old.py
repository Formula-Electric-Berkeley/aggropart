...
def _proc_dk_item(bom, bom_item, bom_idx, dk_items, search_kw=None):
    """TODO document this"""
    ...
    try:
        if search_kw:
            search_request = KeywordSearchRequest(keywords=search_kw, record_count=10)
            dk_resp = digikey.keyword_search(body=search_request)
            common.wait_resp(f'Found the above {dk_resp.products_count} digikey items matching {search_kw}. '
                             'Enter the desired item\'s index',
                             ['*'])
            dk_item = ""
        else:
